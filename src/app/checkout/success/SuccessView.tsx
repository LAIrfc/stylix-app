      ...
      <div className="border border-ivory/10 bg-ink-soft/20 px-7 py-6">
        <div className="grid grid-cols-2 gap-6 sm:grid-cols-3">
          ...
          <div>
            ...
            <p className="text-sm text-ivory/70">{formatDate(order.placed_at)}</p>
          </div>
          <div>
            <p className="text-[9px] uppercase tracking-[0.3em] text-gold/50 mb-1">Payment</p>
            <p className="text-sm text-ivory/70">Stripe Checkout</p>
          </div>
        </div>
      </div>
            ...
            <div key={`${item.id}-${i}`} className="flex gap-5 border border-ivory/8 p-4">
              <div className="flex flex-1 items-start justify-between gap-4 min-w-0">
                <div className="min-w-0">
                  ...
                  {item.qty > 1 && (
                    <p className="text-[9px] text-ivory/30 mt-1">Qty: {item.qty}</p>
                  )}
                </div>
      ...
      <div className="border border-ivory/10 bg-ink-soft/20 px-7 py-6 space-y-3 text-sm">
        ...
        <div className="flex justify-between">
          ...
          <span className={order.shipping_free ? "text-gold/70" : "text-ivory/50"}>
            {order.shipping_free ? "Complimentary" : "Calculated"}
          </span>
        </div>
        <div className="flex justify-between">
          <span className="text-ivory/50">Tax</span>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div>
          ...
          <div className="text-sm text-ivory/60 space-y-0.5">
            ...
            <p>{COUNTRY_LABELS[order.shipping_country] ?? order.shipping_country}</p>
          </div>
        </div>
        <div>
